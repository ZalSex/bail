...
// Pure JS replacement for whatsapp-rust-bridge expandAppStateKeys
const _whatsappRustBridge = {
  expandAppStateKeys: (keydata) => {
    const crypto = require("crypto");
    const key = Buffer.isBuffer(keydata) ? keydata : Buffer.from(keydata);
    const expand = (info) => crypto.createHmac("sha256", key).update(info).digest();
    return {
      indexKey: expand(Buffer.from("index")),
      valueEncryptionKey: expand(Buffer.from("value-encryption")),
      valueMacKey: expand(Buffer.from("value-mac")),
      snapshotMacKey: expand(Buffer.from("snapshot-mac")),
      patchMacKey: expand(Buffer.from("patch-mac"))
    };
  }
};
